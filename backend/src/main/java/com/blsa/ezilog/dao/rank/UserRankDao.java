...
public interface UserRankDao extends JpaRepository<UserRank, String> {
    @Query(value = "select t1.nickname as nickname, count(p.writer) as post_count, t1.rc as reply_count, t1.point as point,t1.level as level\r\n" +
            "from (select count(r.writer) as rc, nickname,point,level from user u left join reply r on u.nickname = r.writer group by u.uid) t1\r\n" +
