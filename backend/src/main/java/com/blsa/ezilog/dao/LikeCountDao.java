...
public interface LikeCountDao extends JpaRepository<LikeCount, BigInteger> {
    ...
    Optional<LikeCount> checkExistLikeCount(Long uid, String type, BigInteger postno);
    ...
    Long countTotal(BigInteger postno, String type);

    @Query(value = "SELECT * FROM like_count WHERE post_no=:postno AND uid=:userid", nativeQuery = true)
    Optional<LikeCount> findLikeCountByUid(BigInteger postno, Long userid);
}
