export function sha256(s) {
    ...
    function cs(m, l) {
        ...
        for (let i = 0; i < m.length; i += 16) {
            ...
            for (let j = 0; j < 64; j++) {
                if (j < 16) W[j] = m[j + i];
                else W[j] = z(z(z(g1(W[j - 2]), W[j - 7]), g0(W[j - 15])), W[j - 16]);

                T1 = z(z(z(z(h, s1(e)), Ch(e, f, g)), K[j]), W[j]);
                T2 = z(s0(a), Maj(a, b, c));
    ...
    function sb(str) {
        let bin = Array();
        const mask = (1 << cz) - 1;
        for (let i = 0; i < str.length * cz; i += cz) {
            bin[i >> 5] |= (str.charCodeAt(i / cz) & mask) << (24 - i % 32);
        }
    ...
    function bh(binarray) {
        const hex_tab = hc ? "0123456789ABCDEF" : "0123456789abcdef";
        let str = "";
